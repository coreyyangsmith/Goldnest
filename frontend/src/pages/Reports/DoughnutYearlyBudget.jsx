//-------------------------------------------------------//
//  File Name: DoughnutYearlyBudget.jsx
//  Description: Generates Charts.js Doughnut Report given Budget data.
//
//  Parents:
//      - Reports.jsx
//
...
const DoughnutYearlyBudget = (props) => {
  ...
  useEffect(() => {
    ...
    // Process budget to sum by year, group budget numbers by main category
      const groupedBudgetByYear = props.budgets.filter(function(row) {
        return row.year == props.selectedYear;
      });

      const sumByMain = groupedBudgetByYear.reduce((acc, cur) => {
        acc[cur.sub_category.main_category.name] = acc[cur.sub_category.main_category.name] + cur.amount || cur.amount;
        return acc;
      }, {})
...
export default DoughnutYearlyBudget