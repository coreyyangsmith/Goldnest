...
const DashboardBudgetOverview = (props) => {
	...
	const [budgetSliderPercentage, setBugetSliderPercentage] = useState(
		budgetSliderCurr / budgetSliderMax
	);
	const [entrySliderPercentage, setEntrySliderPercentage] = useState(
		budgetSliderCurr / budgetSliderMax
	);
	...
	const [budgetSpendPerDayRemaining, setBudgetSpendPerDayRemaining] =
		useState(0);
	const [expenseSpendPerDay, setExpenseSpendPerDay] = useState(0);
	const [expenseSpendPerDayRemaining, setExpenseSpendPerDayRemaining] =
		useState(0);
	...
	useEffect(() => {
		...
		const myCurrBudget = Math.round(
			mySummedBudget * (daysPassed / daysInMonth)
		);
		...
		setBudgetSpendPerDayRemaining(
			parseFloat(reminingBudget / daysRemaining).toFixed(2)
		);
		...
		const myCurrEntryPercentage = Math.round(
			(mySummedEntries / myMaxBudget) * 100
		);
		...
		setExpenseSpendPerDayRemaining(
		...
		if (timeframe === 'Past') {
			...
			setExpenseSpendPerDay(
				parseFloat(mySummedEntries / daysPassed).toFixed(2)
			);
			...
			setBudgetSpendPerDay(0);
			setBudgetSpendPerDayRemaining(
				parseFloat(myMaxBudget / daysRemaining).toFixed(2)
			);
