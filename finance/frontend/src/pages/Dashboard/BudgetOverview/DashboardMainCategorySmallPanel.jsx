...
const DashboardMainCategorySmallPanel = (props) => {
	...
	const handleClick = (mainCategory) => {
		// Deselect
		if (props.selectedMain === mainCategory) props.setSelectedMain('');
		else props.setSelectedMain(mainCategory); // set Selected
	};
	...
	useEffect(() => {
		...
		const mySummedEntries = filteredEntries.reduce(
		...
		const filteredBudgets = budgetByMonth.filter((budget) => {
			return budget.sub_category.main_category.name === props.mainCategory.name;
		});

		const mySummedBudget = filteredBudgets.reduce(
	...
	function obtainCircleColor(entry, budget) {
		...
		if ((percentage < 0.2) | isNaN(percentage))
			//purple
			return '#dacbe6';
		...
		<React.Fragment key={props.mainCategory.id}>
			<Card
				className={`Dashboard-Category ${
					props.selectedMain.id === props.mainCategory.id && 'active'
				}`}
				sx={{
					display: 'flex',
					alignItems: 'center',
				...
				<CardActionArea onClick={() => handleClick(props.mainCategory)}>
					...
					>
						...
						>
							<Box
								sx={{
									background: obtainCircleColor(summedEntries, summedBudget),
									borderRadius: '50%',
									width: '50px',
									height: '50px',
									display: 'flex',
							...
							<Typography
								variant="dashboard_card_heading"
								style={{
									flexWrap: 'wrap',
									height: 'auto',
									width: 'maxContent',
							...
							<Typography variant="dashboard_card_heavy">
								${summedEntries} /
							</Typography>
							<Typography variant="dashboard_card_light">
								${summedBudget}
							</Typography>
						</Stack>
					</CardContent>
