...
const DashboardHeatmap = (props) => {
  ...
  const [calendarMonth, setCalendarMonth] = useState(props.selectedYear + "-" + props.selectedMonth.toString().padStart(2, "0"))
  const [maxValue, setMaxValue] = useState(10000)

  // Load Budget Data and Partiton into Monthly Heatmap Format
  useEffect(() => {
    // Regenerate Calendar Month upon useEffect Trigger
    setCalendarMonth(props.selectedYear + "-" + props.selectedMonth.toString().padStart(2, "0"));
    ...
    if (props.selectedYear !== "" && props.selectedMonth !== "")
    {
      ...
      function getEntriesByDay() {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
          let date = props.selectedYear + "-" + props.selectedMonth.toString().padStart(2, "0") + "-" + day.toString().padStart(2, "0");  

          if (maxVal < entriesByDay.get(day))
            maxVal = entriesByDay.get(day);

          if (entriesByDay.get(day) !== undefined)
            entryArray.push([date,Number(entriesByDay.get(day).toFixed(2))]);
          else
            entryArray.push({"Date": date, "value": 0});
        }
        setMaxValue(maxVal);
