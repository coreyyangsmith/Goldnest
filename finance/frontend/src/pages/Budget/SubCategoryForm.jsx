...
const SubCategoryForm = (props) => {
  ...
  useEffect(() => {
    if (
      props.selectedMain === undefined ||
      props.selectedMain === "" ||
      props.mainCategories === undefined ||
      props.mainCategories === ""
    ) {
      console.log("No Main Category Selected - cannot populate Sub Categories");
      setMainCatName("");
    } else {
      var match = props.mainCategories.filter(
        (obj) => obj.id === props.selectedMain.id
      );
  ...
  const onSubmit = async (FieldValues) => {
    try {
      ...
      const newDataFiltered = newData.data.filter(
        (data) => data.main_category.id == props.selectedMain.id
      );
      ...
      const filteredBudgets = allBudgets.data.filter(
        (data) => data.sub_category.pk === myNewCat.pk
      );
  ...
  return (
    ...
    >
      <Stack marginTop={2} spacing={2} fullwidth="true">
